...
def sparkline(values: List[float], width: int = 40) -> str:
    ...
    n = len(values)
    if n > width:
        # sottocampiona in modo uniforme
        step = n / width
...
def build_layout(city_label: str, data: dict, hours: int, temp_unit: str, wind_unit: str) -> Layout:
    ...
    # FORECAST COMPATTO (prime 12 ore ogni 3h)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Meteo fullscreen tipo htop (auto-refresh, Ctrl+C per uscire)",
    ...
    p.add_argument("--refresh", "-r", type=int, default=30, help="Secondi tra un refresh e l'altro")
    p.add_argument("--refetch", type=int, default=300, help="Secondi tra un nuovo fetch meteo")
    return p.parse_args()

def main() -> int:
    args = parse_args()
    hours = max(6, min(168, args.hours))  # un minimo per i grafici
